...
def ns_fit(real_prices, cashflows, maturities, x0=None):
    ...
    return minimize(ns_error, x0, args=(real_prices, cashflows, maturities), method='powell')
...
class NelsonSiegel:
    ...
    def convexity(self, cashflows, cashflow_maturities):
        return self.gamma(cashflows, cashflow_maturities) / self.price(cashflows, cashflow_maturities)
...
def vasicek(eta, gamma, r0, sigma, maturities):
    ...
    a = 1 / pow(gamma, 2) * (b - maturities) * (eta * gamma - 0.5 * sigma2) - sigma2 * pow(b, 2) / (4 * gamma)
